...
public class FileUtil {
	...
	public static void setUserId (String userId) {
		mUserId = userId;
		LogUtil.d(FileUtil.class, "***UserId = " + mUserId + "***");
	}
...
//	}
   ...
   public static void copyAssets2SD(Context context, String assetDir, String outDir) {
	...
	/**
	 * 描述：初始化存储目录.(每个登录用户都有一个单独的存储目录),alter by sunsl, 2015/12/29;
	 *
	 * @param context the context
	 */
	public static void initFileDir(Context context){
		LogUtil.d(FileUtil.class, "***********************initFileDir------***UserId = " + mUserId + "***");
		...
		if (null != mUserId && !mUserId.equals("")) {
			//默认下载文件根目录. 
			downloadRootPath = downloadRootPath +  mUserId + File.separator;
		}
...
//				fileDownloadDir = fileDownloadDirFile.getPath();
				fileDownloadDir = root.getAbsolutePath() + fileDownloadPath;
	...
	public static String getFileDownloadDir(Context context) {
		...
		LogUtil.d(FileUtil.class, fileDownloadDir);
	...
	/**
	 * Gets the db download dir.  added by sunsl, 2015/12/29;
	 *
	 * @param context the context
	 * @return the db download dir
	 */
	public static String getDbDownloadDir(Context context, String userId) {
		if(downloadRootDir == null){
			initFileDir(context);
		}
		return dbDownloadDir;
	...
	public static String transEncodeUrl(String url) {
		String translatedPath = "";
		try {
			translatedPath = URLEncoder.encode(url, "utf-8")
